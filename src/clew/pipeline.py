...
from clew.correction import correct_transcript
...
def _create_recorder(config: Config):
    ...
    from clew.audio.sounddevice_recorder import SoundDeviceRecorder
...
def _create_transcriber(config: Config, progress=None):
    ...
    if config.transcription.engine == "canary_mlx":
        from clew.transcription.canary_mlx_transcriber import CanaryMlxTranscriber, is_available
...
def _format_output(config: Config, transcript_result, summary_text: str | None = None) -> tuple[str, str | None]:
    ...
    if config.output.format == "json":
        ...
        from clew.output.markdown import format_summary, format_transcript
...
def run_watch(config: Config, sustained_seconds: float) -> None:
    ...
    from clew.audio.coreaudio import _find_binary
