...
- **BUG3 (open): mic↔system track offset -28s scrambles transcript order.** First real solo test: the two async capture engines' start-time delta is computed wrong → Owner line at [59:51], transcript out of order. Under investigation. Lesson: the two-physical-source design is only as good as the sync between them; validate the offset against acoustic cross-correlation, not just host-clock deltas.
