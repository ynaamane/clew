...
Method, for reuse: `open "clew://library"` →
`bash scripts/ui-evidence/capture.sh Clew /tmp/ui-ev` → Read the PNG. Window-scoped by
...
**UPDATE 2026-08-24 (BMAD audit) — the stale-premise flag above was vindicated and sharpened**: the
newer crash TODO.md later cited against MPS (pytorch#181650) involves a Metal kernel family added to
torch AFTER the 2.8.0 pinned here (kernel landed Dec 2025, torch 2.8.0 published Aug 2025), and both
candidate fix PRs upstream were closed unmerged — so MPS on this stack is UNTESTED, not proven
broken. HF token and real clips are now available; the decision point is running
`scripts/verify_with_token.py check_mps_vs_cpu_diarization`, extended with per-centroid norm+cosine
cells (segment-set equality cannot detect the zero-norm-centroid hazard found the same day). ~9-13x
diarization speedup in play. Full verdict: TODO.md § 2026-08-24.

**UPDATE 2026-08-25 — the gate ran, the flip landed.** `check_mps_vs_cpu_diarization` passed:
bit-fidelic CPU/MPS parity on a real clip (84/84 segments, per-cluster embedding cosine
1.0) and a 29.5x measured speedup. `diarization.device` (`"auto"`/`"cpu"`/`"mps"`, default
`"auto"`) is back in `DiarizationConfig`, this time with a `_resolve_diarization_device()`
that never crashes a machine without MPS (graceful CPU fallback + warning) and a config-load
validator rejecting anything else (`de08f45`, `9b89e1d`, `739aa94`). docs/configuration.md's
"Diarization always runs on CPU" line is updated accordingly.
