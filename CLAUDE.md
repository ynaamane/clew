...
- **`mock.patch.dict("sys.modules", {...})` snapshots the ENTIRE dict** and restores that exact
  snapshot on exit — a module imported for the FIRST time inside the block (torch, via any
  production import) is silently erased from `sys.modules` afterward, and a LATER test's
  unrelated import then fails (`RuntimeError: ... already has a docstring`), several tests
  downstream of the culprit. Import the module ABOVE the patch (`import torch  # noqa: F401`);
  the pattern is already in `test_diarize_unwraps_speaker_diarization_from_diarize_output`.
